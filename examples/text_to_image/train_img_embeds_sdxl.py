...
def main(args):
    ...
    if args.train_text_encoder:
        ...
        image_encoder_one.add_adapter(text_lora_config)
        image_encoder_two.add_adapter(text_lora_config)
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                if random.random() < 0.5:
                    conditioning_embeds = prompt_embeds
                    pooled_conditioning_embeds = pooled_prompt_embeds
                else:
                    conditioning_embeds = projected_image_embeds
                    pooled_conditioning_embeds = projected_pooled_image_embeds

                unet_added_conditions.update(
                    {"text_embeds": pooled_conditioning_embeds}
                )
                model_pred = unet(
                    noisy_model_input,
                    timesteps,
